...
async def get_notes(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    notes = await repository_notes.get_all_notes(skip, limit, db)
    return notes
...
async def create_note(note: NoteBase, db: Session = Depends(get_db)):
    note = await repository_notes.create_note(note, db)
    return note


@router.put("/{note_id}", response_model=NoteResponse)
async def update_note(note_id: int, note: NoteUpdate, db: Session = Depends(get_db)):
    note = await repository_notes.update_note(note_id, note, db)
    if note is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Not Found')
    return note
...
async def update_status_note(note_id: int, note: NoteStatus, db: Session = Depends(get_db)):
    note = await repository_notes.update_status_note(note_id, note, db)
    if note is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Not Found')
    return note
